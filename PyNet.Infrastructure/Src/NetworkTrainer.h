...
private:
	...
	std::unique_ptr<Matrix> GetdError_dWeight(std::unique_ptr<Matrix>& weightMatrix, std::unique_ptr<Vector>& inputLayer, std::unique_ptr<Vector>& outputLayer);
	double GetdError_dBias(std::unique_ptr<Vector>& outputLayer);
...
public:
	...
	void UpdateWeights(std::vector<std::unique_ptr<Matrix>>& weightMatrices, std::vector<std::unique_ptr<Vector>>& biases, double learningRate, bool reverse = false);
};
